...
public interface ISessionBuilder<T1, T2, T3> : ISessionBuilder
    where T1 : notnull
    where T2 : notnull
    where T3 : notnull
{
    ...
    /// <summary>
    /// Declare additional service (max 4 services)
    /// </summary>
    /// <typeparam name="T4">Fourth service type</typeparam>
    new ISessionBuilder<T1, T2, T3, T4> WithService<T4>() where T4 : notnull;
    ...
    void Execute(Action<T1, T2, T3> action, Action<Exception>? onError = null);
    ...
    /// <param name="action">Action to execute</param>
    /// <param name="onError">Optional error handler. If not provided, defaults to Rollback() for database sessions</param>
    Task ExecuteAsync(Func<T1, T2, T3, Task> action, Action<Exception>? onError = null);
    ...
    /// <param name="onError">Optional error handler. If not provided, defaults to Rollback() for database sessions</param>
    TResult ExecuteWithResult<TResult>(Func<T1, T2, T3, TResult> func, TResult defaultValue = default!, Action<Exception>? onError = null);
    ...
    /// <param name="defaultValue">Default value to return if exception occurs</param>
    /// <param name="onError">Optional error handler. If not provided, defaults to Rollback() for database sessions</param>
    Task<TResult> ExecuteWithResultAsync<TResult>(Func<T1, T2, T3, Task<TResult>> func, TResult defaultValue = default!, Action<Exception>? onError = null);
}
